...
async function connect() {
    ...
    try {
        ...
        if (!identity) {
            console.error(`❌ Identity ${identityName} not found in wallet`);
            console.log('💡 Please run: node enrollAdmin.js && node registerUser.js');
            process.exit(1);
        }
    ...
    setInterval(() => {
        ...
        Object.keys(intersections).forEach(id => {
            ...
            if (state.decision && state.decision.isEmergency && !state.emergencyOverride) {
                ...
                console.log(`[${id}] 🚨 EMERGENCY INTERRUPT! Forcing immediate switch to ${emergencyPhase}`);
                
                state.emergencyOverride = true;
                state.currentPhase = emergencyPhase;
                state.targetPhase = emergencyPhase;
                state.state = emergencyPhase === 'NS' ? 'GREEN_NS' : 'GREEN_EW';
                state.timeRemaining = state.decision.greenDuration || 120;
                state.greenDuration = state.decision.greenDuration || 120;
            ...
            if (state.decision) {
                ...
                if (blockchainPhase !== state.targetPhase && !state.inTransition) {
                    console.log(`[${id}] 🆕 New phase: ${blockchainPhase} (${blockchainDuration}s)`);
    ...
    setInterval(async () => {
        ...
        for (const id of Object.keys(intersections)) {
            try {
                ...
                console.log(`🟢 [${id}] Phase: ${decision.phase}, Duration: ${decision.greenDuration}s`);
                console.log(`   Reason: ${decision.algorithmReason}`);

            } catch (err) {
                console.error(`❌ Blockchain error for ${id}:`, err.message);
            }
        }
...
app.post('/api/emergency/:intersectionId', async (req, res) => {
    try {
        ...
        console.log(`🚨 Emergency request: ${vehicleType} at intersection ${intersectionId} direction ${direction}`);
        ...
        if (intersections[intersectionId] && intersections[intersectionId].decision) {
            ...
            if (decision.isEmergency) {
                console.log(`🚨 Forcing immediate light change for ${intersectionId} to ${direction}`);
                
                intersections[intersectionId].currentPhase = decision.phase;
                intersections[intersectionId].targetPhase = decision.phase;
                intersections[intersectionId].state = decision.phase === 'NS' ? 'GREEN_NS' : 'GREEN_EW';
                intersections[intersectionId].timeRemaining = decision.greenDuration || 120;
                intersections[intersectionId].greenDuration = decision.greenDuration || 120;
...
app.post('/api/emergency/clear/:emergencyKey', async (req, res) => {
    try {
        ...
        console.log(`✅ Clearing emergency: ${emergencyKey}`);
...
console.log('🚨 Emergency vehicle endpoints registered');
